...
#define				 CAMERA_LIMIT				 		20                     //Max loaded cameras (keep this as low as possible for the best performance)
...
#define				 CAMERA_DIALOG_RANGE          		16337                    //dialog ID range (Example: 0 will take dialogid's 0 - 9)
...
public OnFilterScriptInit()
{
	...
	print("Initializing...");
	LoadCameras();
	CreateDynamicCP(-1543.3616,-442.5969,6.1000, 1.0,-1,0);
	return 1;
}
...
{
	#pragma unused UseMPH
	static Float:ST[3];
	GetVehicleVelocity(vehicleid,ST[0],ST[1],ST[2]);
	return (1.61*floatsqroot(floatpower(floatabs(ST[0]), 2.0) + floatpower(floatabs(ST[1]), 2.0) + floatpower(floatabs(ST[2]), 2.0)) * 100.3);
/*
...
{
	for(new a;a<MAX_PLAYERS;a++) if(IsPlayerConnected(a))
	{
	    ...
	    for(new b = 0;b<loaded_cameras +1;b++)
	    {
	        ...
	        {
				new Float:Coo[3];
				GetPlayerPos(a,Coo[0],Coo[1],Coo[2]);
				if(Coo[2]+1 > SpeedCameras[b][_z] && Coo[2]-5 < SpeedCameras[b][_z]) 
				{
					new speed = floatround(GetVehicleSpeed(GetPlayerVehicleID(a),SpeedCameras[b][_usemph]));
					new limit = SpeedCameras[b][_limit];
					if(speed > limit)
					{
						if(!IsPoliceCar(GetPlayerVehicleID(a)))
						{
							...
							{
								SendClientMessageEx(a,0xFF1E00FF,"sisis","Virsijote greicio limita! Jus vaziavote ",speed,"km/h, o limitas - ",limit, "km/h.");
								SendClientMessage(a,0xFF1E00FF,"Prasome sumoketi bauda policijai.");
								if(GetPlayerWantedLevel(a)+1 >= 10) SetPlayerWantedLevel(a,10);
								else SetPlayerWantedLevel(a,GetPlayerWantedLevel(a)+1);
								CallRemoteFunction("AchievementMain","iif",a,54,1.0);
							}
						}
					}
				}
...
{
	if(CallRemoteFunction("IsKobraCheetah","i",vehicleid)) return 1;
	switch(GetVehicleModel(vehicleid))
	{
		case 523,430,497,596,597,599,601,427,490,447,470,433,528: return 1; // policijos
		case 481,509,510: return 1;// moto
		case 593,592,577,563,553,548,520,519,513,512,511,
        488,487,476,469,460,425,417,493,484,453: return 1; // skraidantys
		case 537,538: return 1;//traukiniai
		case 407,416,489,544: return 1; //  gaisrine,medikai
