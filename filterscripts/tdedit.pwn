// Start Of File
/*
Zamaroht's TextDraw Editor Modified by MakuPL version 1.0 RC
Designed for SA-MP 0.3d RC5-3 and higher.


Author: Zamaroht (Nicolás Laurito) (Argentina)
Modifier: MakuPL (Patryk Makowski) (Poland)

Changes:
Added full support for texture - Font type 4

Last update: 16 October 2011, 15:27 (GMT-1)
...
separate README file in a .zip file, etc.
If you modify this file, the same terms apply. You have to include the original
author (Zamaroht and MakuPL) and the link back to the mentioned webpage.
...
#define MSG_COLOR           0x00AAEEFF	// Color to be shown in the messages.
...
{
	print("\n--------------------------------------");
	print(" TextDraw Editor by Zamaroht (Support Textures by Maku) Loaded...");
	print("--------------------------------------\n");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    switch(dialogid)
    {
        ...
        case 1579: // Main edition menu
        {
            if(response)
            {
            	if(tData[pData[playerid][P_CurrentTextdraw]][T_Font] == 4)
	        	{
        ...
        case 1591: // Change textdraw's font
        {
            if(response)
            {
            	if(listitem == 4)
				{
					tData[pData[playerid][P_CurrentTextdraw]][T_LastUseBox] = tData[pData[playerid][P_CurrentTextdraw]][T_UseBox];
					tData[pData[playerid][P_CurrentTextdraw]][T_LTextSizeY] = tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeY];
					tData[pData[playerid][P_CurrentTextdraw]][T_LTextSizeX] = tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeX];
				    
					tData[pData[playerid][P_CurrentTextdraw]][T_UseBox] = 1;
					tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeY] = 100.0;
					tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeX] = 100.0;
				...
				}else if(tData[pData[playerid][P_CurrentTextdraw]][T_LastFont] == 4){

                    tData[pData[playerid][P_CurrentTextdraw]][T_UseBox] = tData[pData[playerid][P_CurrentTextdraw]][T_LastUseBox];
					tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeY] = tData[pData[playerid][P_CurrentTextdraw]][T_LTextSizeY];
					tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeX] = tData[pData[playerid][P_CurrentTextdraw]][T_LTextSizeX];

					SaveTDData(pData[playerid][P_CurrentTextdraw], "T_UseBox");
					SaveTDData(pData[playerid][P_CurrentTextdraw], "T_TextSizeX");
					SaveTDData(pData[playerid][P_CurrentTextdraw], "T_TextSizeY");
				}
				tData[pData[playerid][P_CurrentTextdraw]][T_LastFont] = listitem;
	...
	switch(dialogid)
	{
	    ...
	    case 5:
	    {
	        ...
	        if(tData[pData[playerid][P_CurrentTextdraw]][T_Font] != 4)
	        {
				...
				if(strfind(tData[pData[playerid][P_CurrentTextdraw]][T_Text], ":") == -1 && strlen(tData[pData[playerid][P_CurrentTextdraw]][T_Text]) > 4)
				{
					ShowTextDrawDialog(playerid, 8);
             	}else{
             		format(info, sizeof(info), "%sChange texture\n", info);
		        	format(info, sizeof(info), "%sChange position\n", info);
		        	format(info, sizeof(info), "%sChange texture color\n", info);
		        	format(info, sizeof(info), "%sChange font\n", info);
		        	format(info, sizeof(info), "%sChange size\n", info);
		        	format(info, sizeof(info), "%sDuplicate Textdraw...\n", info);
		        	format(info, sizeof(info), "%sDelete Textdraw...", info);
	    ...
	    case 8:
	    {
	        if(tData[pData[playerid][P_CurrentTextdraw]][T_Font] != 4)
	        {
				...
				new info[1024];
	        	info = "Write the new textdraw's name texture. The current texture is:\n\n";
	        	format(info, sizeof(info), "%s%s\n\nIMPORTANT!\n Enter the name of the texture as \"{FF0000}txdfile{FFFFFF}:{00FF00}name{CCCCCC}\"", info, tData[pData[playerid][P_CurrentTextdraw]][T_Text]);
	        	ShowPlayerDialog(playerid, dialogid+1574, DIALOG_STYLE_INPUT, CreateDialogTitle(playerid, "Textdraw's string"), info, "Accept", "Go back");
...
public OnPlayerKeyStateChange( playerid, newkeys, oldkeys )
{
    if(pData[playerid][P_KeyEdition] != EDIT_NONE && newkeys == KEY_SECONDARY_ATTACK)
	{
        ...
        if(pData[playerid][P_KeyEdition] == EDIT_BOX && tData[pData[playerid][P_CurrentTextdraw]][T_Font] != 4) SetTimerEx("ShowTextDrawDialogEx", 500, 0, "ii", playerid, 24);
